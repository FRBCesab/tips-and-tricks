...
Make sure to select the {{< fa brands r-project >}} language (Figure 2).
Now we can write `ld` and press **`SHIFT`** **`+`** **`TAB`** to write `devtools::load_all()`.
